...
void Table::updateButtonStates() {
	...
	switch (checkButtonState2(buttonMutePump)) {
	case 0:
		break;
	...
	switch (checkButtonState2(buttonBrightness)) {
	case 0:
		break;
...
void Table::playAnimation() {
	...
	case 32:
		break;
	case 33:
		callAnimationMethod(&SingleStripeAnimationPlayer::playJuggle, backLeds);
	...
	//void (SingleStripeAnimationPlayer::*animationMethod)(LedPart&) = &SingleStripeAnimationPlayer::playJuggle;

}

void Table::callAnimationMethod(void (SingleStripeAnimationPlayer::*animationMethod)(LedPart&), LedPart &part ...) {
	//vector<LedPart> data = std::forward<LedPart>(parts)...;
//	LedPart part();

	(player.*animationMethod)(part);

//	LedPart partone= part[0];
}
